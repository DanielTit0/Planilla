...
CREATE PROCEDURE ObtenerInformacionPlanilla(
    IN co_empl INT,
    IN plan_month INT,
    IN plan_year INT,
    IN co_tipo_plan INT
...
BEGIN
    SELECT
        (SELECT de_cpto FROM concepto_planilla WHERE co_cpto = pe.co_cpto) AS de_cpto,
        pe.monto,
        pe.co_tipo_plan,
        pe.fe_plan,
        pe.fe_pago,
        pe.fe_migra
    FROM
        planilla_empleado pe
    WHERE
        pe.co_empl = co_empl
        AND MONTH(pe.fe_plan) = plan_month
        AND YEAR(pe.fe_plan) = plan_year
